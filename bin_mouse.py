...
binary_numbers_vial = []
print('Вычисляем номера пробирок в двоичном коде')
for num in tqdm(range(1, 1001)):
    binary_numbers_vial.append(dec_to_bin(num, bit_depth=10))

print('\nДаём по капле из пробирки мышам.\nЕдиницы - клетки с мышами, которым нужно дать каплю')
...
print('\nУмерли мыши в клетках №{}'.format(' №'.join([str(number_mouse) for number_mouse in dead_mouse])))
print('Отмечаем номера погибших мышей единицей справа налево, получается {}'.format(number_vial_poison))
print('\nПереводим {} в десятичное число.\nПолучается {}, это номер пробирки с ядом'.format(number_vial_poison, bin_to_dec(number_vial_poison)))
